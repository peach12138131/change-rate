...
import time
from auto_article_config_avigo import keyword_list,claude_key,tavily_key,openai_key,generate_svg_prompt,rewrite_prompt,extract_svg_from_text, extract_final_report,news_schema,seo_metadata,seo_keywords,seo_link,seo_rewrite_prompt
...
def auto_write_article(news_list):
    ...
    try:
        ...
        #保存news_pool为json
        news_file_name=os.path.join(output_dir, "avigo_news_pool.json")
            ...
            for category, news_list in news_by_category.items():
                ...
                while start_idx < len(news_list):
                    ...
                    if rewritten_article:
                        ...
                        #保存seo文章
                        jetbay_seo_dir=os.path.join(output_dir,"AVIGO")
                        os.makedirs(jetbay_seo_dir, exist_ok=True)

                        seo_filename = os.path.join(output_dir, 'AVIGO', f'seo_article_{batch_count + 1}_{timestamp}.txt')
